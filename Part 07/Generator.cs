...
public class Generator : MonoBehaviour
{
    ...
    private void UpdateDisplay()
    {
        ...
        moneyCostDisp.text = currencyManager.SciNotToUSName(moneyCost * amount);
        limiterCostDisp.text = currencyManager.SciNotToUSName(limiterCost * amount);
        ...
        {
            productCostDisp.text = currencyManager.SciNotToUSName(productCost * amount);
    ...
    {
        BigDouble quantityToBuy = currencyManager.money / moneyCost;
        int buyAmountIndex = Array.IndexOf(generatorManager.buyAmounts, generatorManager.currentBuyAmount);

        if(quantityToBuy > currencyManager.limiter / limiterCost)
        {
            quantityToBuy = currencyManager.limiter / limiterCost;
        }

        if(hasProduct && quantityToBuy > product.quantity / productCost)
        {
            quantityToBuy = product.quantity / productCost;
        }
    ...
    public void Buy()
    {
        ...
        quantity += buyAmount;
        currencyManager.money -= buyAmount * moneyCost;
        currencyManager.limiter -= buyAmount * limiterCost;

        if (hasProduct)
        {
            product.quantity -= buyAmount * productCost;
        }
    }
}
